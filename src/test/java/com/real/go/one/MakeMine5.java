...
public class MakeMine5 {
	...
	public List<String[]> solveNQueeens(int row, int n, String[] result,
			List<String[]> re) {
	...
	public List<Integer> spiralOrder(int[][] matrix) {
		...
		return spiralOrder(matrix, 0, row - 1, 0, column - 1,
				new ArrayList<Integer>());
	}

	public List<Integer> spiralOrder(int[][] matrix, int rs, int re, int cs,
			int ce, List<Integer> result) {
		if (rs > re || cs > ce) {
			return result;
		}
		for (int i = cs; i <= ce; i++) {
	 ...
	 * 2: Given [1,2],[3,5],[6,7],[8,10],[12,16], insert and merge [4,9] in as
	 * [1,2],[3,10],[12,16]. This is because the new interval [4,9] overlaps
	 * with [3,5],[6,7],[8,10].
	 */
	public List<Interval> insert(List<Interval> intervals, Interval newInterval) {
		...
		if (start == end) {
			...
			result.add(new Interval(Math.min(newInterval.start,
					intervals.get(start).start), Math.max(newInterval.end,
					intervals.get(end - 1).end)));
