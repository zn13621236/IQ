...
public class MakeMine1 {
	 ...
	 * triplets. For example, given array S = {-1 0 1 2 -1 -4}, A solution set
	 * is: (-1, 0, 1) (-1, -1, 2)
	 */
	...
	public int threeSumClosest(int[] num, int target) {
		...
		for (int i = 0; i < num.length - 2; i++) {
			...
			while (x < y) {
				int newT = t - num[x];
				if (Math.abs(newT - num[y]) < min) {
					min = Math.abs(newT - num[y]);
					result = num[i] + num[x] + num[y];
				}
	 ...
	 * telephone buttons) is given below. Input:Digit string "23" Output: ["ad",
	 * "ae", "af", "bd", "be", "bf", "cd", "ce", "cf"].
	 */
	public List<String> letterCombinations(String digits) {
		// if(digits.length()==1){
		// List<String> aList= getCom(digits.charAt(0));
		// return aList;
		// }
		// List<String> result= letterCombinations(digits.substring(1));
		// List<String> a= getCom(digits.charAt(0));
		// for(String s:a){
		//
		// }
		return null;
	...
	private List<String> getCom(char digits) {
	 ...
	 * solution set must not contain duplicate quadruplets. For example, given
	 * array S = {1 0 -1 0 -2 2}, and target = 0. A solution set is: (-1, 0, 0,
	 * 1) (-2, -1, 1, 2) (-2, 0, 0, 2)
	 */
		...
		for (int i = 0; i < len - 3; i++) {
			...
			for (int j = i + 1; j < len - 2; j++) {
				...
				while (x < y) {
					if (t > num[x] + num[y]) {
						x++;
					} else if (t < num[x] + num[y]) {
						y--;
					} else {
						List<Integer> aList = new ArrayList<>();
						aList.addAll(Arrays.asList(num[i], num[j], num[x],
								num[y]));
						result.add(aList);
					}
				}
	...
	public boolean isValid(String s) {
		...
		for (int i = 1; i < s.length(); i++) {
			if (check(s.charAt(i), stack.peek())) {
				stack.pop();
			} else {
	...
	private boolean check(char c, char peek) {
		if (c == '(' && peek == ')')
			return true;
		if (c == '[' && peek == ']')
			return true;
